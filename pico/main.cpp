...
int main() {
    ...
    myTFT.TFTdrawText(5, 80, "B:", ST7735_TAN, ST7735_BLACK, 2);
...
while (1) {
    ...
    U_avg /= 110.0;
    double B_avg = (U_avg - 250.66)/9.7388;
    string u_value = format("{:.4f}  ", U_avg);
    string b_value = format("{:.4f}  ", B_avg);

    myTFT.TFTdrawText(40, 40, u_value, ST7735_GREENYELLOW, ST7735_BLACK, 2);
     myTFT.TFTdrawText(40, 80, b_value, ST7735_TAN, ST7735_BLACK, 2 );
    string filename;
    string datetime = "";
    //string temps = "";
    is_rtc = rtc::ds3231_scan();
    ...
    if(is_sd) log_data(datetime + u_value + b_value, filename);
